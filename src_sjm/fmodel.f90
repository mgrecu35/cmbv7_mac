...
subroutine get_state_clw1d_rh(nvar,neof,nlev,x,xbar,xstd,prof_W,prof_avg,prof_std,prof_eofs,gdata)
  ...
  if(gdata%anc_clwp .gt. 0.) then
    do i=1,nlev
      if((isnan(gdata%anc_clw_lev(i+MERRA_NLEV-nlev))).eqv..False.) gdata%cloud_water(i) = 1e3*gdata%anc_clw_lev(i+MERRA_NLEV-nlev)/gdata%anc_clwp*clwp
...
subroutine get_state_clw2d_rh(nvar,neof,nclw,nlev,x,xbar,xstd,prof_W,prof_avg,prof_std,prof_eofs,clwprofs,gdata)
  ...
  do i=1,nclw
    ...
    if(gdata%anc_clwp .gt. 0.) then
      do j=1,nlev
        if((isnan(gdata%anc_clw_lev(j+MERRA_NLEV-nlev))).eqv..False.) clwprofs(i,j) = 1e3*gdata%anc_clw_lev(j+MERRA_NLEV-nlev)/gdata%anc_clwp*clwp
