...
subroutine read_RSS_tables(dirname)
  ...
  open(unit=3,file=file_coeffs_wind_isotropic,form='unformatted',status='old',action='read')
  read(3) LUT%acoef
  close(3)
  
  write(file_coeffs_wind_direction, '(A,A)') trim(dirname), 'fit_emiss_phir_wind.dat'
  open(unit=3,file=file_coeffs_wind_direction,form='unformatted',status='old',action='read')
  read(3) LUT%bcoef
  close(3) 
  
  write(file_em0_ref_freq_sst, '(A,A)') trim(dirname), 'em0_ref_freq_sst.dat'
  open(unit=3,file=file_em0_ref_freq_sst,status='old',form='unformatted',action='read')
  read(3) LUT%rcoef
  close(3)
  
  write(file_coeffs_sctterm, '(A,A)') trim(dirname), 'mk_scatterm_table_all.dat'
  open(unit=3,file=file_coeffs_sctterm,status='old',form='unformatted',action='read')
  read(3) LUT%scatterm
  close(3)
  !stop
